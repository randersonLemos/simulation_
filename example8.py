...
if __name__ == '__main__':
    ...
    sim_folder_group = pathlib.Path('SIM_ICV_05_STG')
    sims = []
    for idx, qty in enumerate(qtys):
        ...
        for prod in prod_lst:
            ...
            maxx = qty[1]
            inte = (maxx-minn) / 4
            icvv.add_rule([qty[0], kw.greater_than(),  minn         , 0.80])
            icvv.add_rule([qty[0], kw.greater_than(),  minn +   inte, 0.60])
            icvv.add_rule([qty[0], kw.greater_than(),  minn + 2*inte, 0.40])
            icvv.add_rule([qty[0], kw.greater_than(),  minn + 3*inte, 0.20])
            icvv.add_rule([qty[0], kw.greater_than(),  minn + 4*inte, 0.00])
